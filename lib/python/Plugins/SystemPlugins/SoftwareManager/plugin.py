...
config.plugins.configurationbackup.backupdirs = ConfigLocations(default=[eEnv.resolve('${sysconfdir}/enigma2/'), '/etc/auto.network', '/etc/enigma2/automounts.xml', '/etc/default/dropbear', '/home/root/.ssh/', '/etc/samba/', '/etc/fstab', '/etc/inadyn.conf', '/etc/network/interfaces', '/etc/wpa_supplicant.conf', '/etc/wpa_supplicant.ath0.conf', '/etc/wpa_supplicant.wlan0.conf', '/etc/resolv.conf', '/etc/default_gw', '/etc/hostname'])
...
class UpdatePluginMenu(Screen):
	...
	def __init__(self, session, args = 0):
		...
		if self.menu == 0:
			...
			if not boxtype.startswith('az') and not boxtype.startswith('dm') and not brandoem.startswith('cube') and not brandoem.startswith('wetek'):
